...
void compareCFpap (int kT = 0, const char* system, int cent)
{
    ...
    TFile *ff = new TFile(Form("cf_%s_f3_kT%d.root",system,kT), "read");

    TFile *foutp = new TFile(Form("cf_%s_f1_kT%d_outP.root",system,kT), "read");
    TFile *foutn = new TFile(Form("cf_%s_f1_kT%d_outN.root",system,kT), "read");
    // TFile *fsidep = new TFile(Form("cf_%s_f1_kT%d_sideP.root",system,kT), "read");
    // TFile *fsiden = new TFile(Form("cf_%s_f1_kT%d_sideN.root",system,kT), "read");
    // TFile *flongp = new TFile(Form("cf_%s_f1_kT%d_longP.root",system,kT), "read");
    // TFile *flongn = new TFile(Form("cf_%s_f1_kT%d_longN.root",system,kT), "read");
    ...
    // for (int j = 0; j < 5; j += 2) {
        ...
        summed = (TH1D*)ff->Get(Form("corrfun_c%d",cent));

        coutp = (TH1D*)foutp->Get(Form("corrfun_c%d",cent));
        coutn = (TH1D*)foutn->Get(Form("corrfun_c%d",cent));
        // csidep = (TH1D*)fsidep->Get(Form("corrfun_c%d",cent));
        // csiden = (TH1D*)fsiden->Get(Form("corrfun_c%d",cent));
        // clongp = (TH1D*)flongp->Get(Form("corrfun_c%d",cent));
        // clongn = (TH1D*)flongn->Get(Form("corrfun_c%d",cent));

        coutp->SetBit(TH1::kIsAverage);
        coutn->SetBit(TH1::kIsAverage);
        // csidep->SetBit(TH1::kIsAverage);
        // csiden->SetBit(TH1::kIsAverage);
        // clongp->SetBit(TH1::kIsAverage);
        // clongn->SetBit(TH1::kIsAverage);
        ...
        summed->SetMarkerStyle(24);
        summed->SetMarkerSize(1.9);
        summed->SetMarkerColor(kBlack);

        coutp->SetMarkerStyle(20);
        coutp->SetMarkerSize(1.3);
        coutp->SetMarkerColor(kBlack);

        // csidep->SetMarkerStyle(24);
        // csidep->SetMarkerSize(1.9);
        // csidep->SetMarkerColor(kBlack);

        // clongp->SetMarkerStyle(28);
        // clongp->SetMarkerSize(2.5);
        // clongp->SetMarkerColor(kBlack);
